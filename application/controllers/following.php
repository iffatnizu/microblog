...
class Following extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('userLogin')) {
            ...
            $data['followingList'] = $this->model_user->getFollowingList($this->session->userdata('userId'));

            $page['header'] = $this->load->view(SiteConfig::COMPONENT_USER_HEADER, $data, true);
            $page['leftConaitner'] = $this->load->view(SiteConfig::COMPONENT_FOLLOWING_LIST, "", true);
